...
bot_ardrone::bot_ardrone(unsigned char id, unsigned char botinterface, unsigned char slam_mode)
{
	...
	/* Line finder */
	linecontroller = new LineFinder( Coordinate( BOT_ARDRONE_FRAME_W, BOT_ARDRONE_FRAME_H ) );
}
...
void bot_ardrone::frame_received(bot_ardrone_frame *f)
{
	...
	if (line_follow_state)
	{
		linecontroller->add_frame(f);
		lineupdate( linecontroller->getControl() );
	}
...
void bot_ardrone::set_follow_line()
{
	if( !this->line_follow_state )
		line_follow_state = true;
	else
		line_follow_state = false;
	std::cout << "Line Follow State " << line_follow_state;
}

/*
void bot_ardrone::test( Mat image )
{
	if( line_follow_state )
	{
		find.add_frame( image );
		double angle = find.getAngle();
		double horizontal  = 0;
		goTo( angle, horizontal );
	}
}
*/


void bot_ardrone::move( float speed , int type )
{
	if (this->control_get( BOT_ARDRONE_Velocity, type ) != speed)
	{
		this->control_set(BOT_ARDRONE_Velocity, type, speed);
		this->control_update();
	}
}

void bot_ardrone::lineupdate( Control update )
{
	std::cout << "Control [rot, lat, lin]: [" << update.rotation << ", " << update.lateral << ", " << update.linear << "]\n";
	//move( update.rotation, BOT_ARDRONE_RotationalVelocity );
	move( update.lateral,  BOT_ARDRONE_LateralVelocity );
	//move( update.linear,  BOT_ARDRONE_LinearVelocity );
}
