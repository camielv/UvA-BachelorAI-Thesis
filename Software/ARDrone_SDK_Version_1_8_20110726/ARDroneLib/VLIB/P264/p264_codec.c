...
C_RESULT p264_encode_blockline( video_controller_t* controller, const vp_api_picture_t* blockline, bool_t picture_complete )
{
  ...
  {
    uint32_t add = 32 - clz((uint32_t) stream->used/controller->blockline);      // estimate the log2 size of a blockline in the stream
  ...
  if (controller->target_size > 0)
  {
    ...
    if (controller->picture_type == VIDEO_PICTURE_INTRA)
      target_size = (uint32_t) ((NB_P_FRAMES + 1)*controller->target_size/(1+NB_P_FRAMES*i_p_ratio));
    else
      target_size = (uint32_t) ((NB_P_FRAMES + 1)*controller->target_size*i_p_ratio/(1+NB_P_FRAMES*i_p_ratio));
