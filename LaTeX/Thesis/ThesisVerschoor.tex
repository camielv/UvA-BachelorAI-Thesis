...
Motion detection is a tool in Computer Vision that is applied to detect whether an object has changed its position relative to its surroundings or the surroundings have changed relative to an object. Motion can be detected by mechanical and electronic methods. This is done by various sensors such as: sound, ultrasonic and opacity. Motion can be detected electrically via optical or acoustical detection. This thesis focuses on optical motion detection from the front camera as the platform has only a limited sensor suite (see chapter \ref{platform}). This paper will focus on a algorithm that has been investigated for obstacle avoidance for the AR-Drone \cite{Jurriaans2011}. In this approach the Optical Flow is calculated over a set of images. The resulting vectors were are to calculate the fundamental matrix using RANSAC, which serves as the input for the Hartley algorithm that generates a disparity map. This algorithm is explained in the following sections.

\subsubsection{Shi-Thomasi}
To find the optical flow in a set of images features have to be extracted from the current frame and be found in the next frame. An possible algorithm that can provide decent features is the Shi-Tomasi algorithm. It proposes the tracking of corners and defines a good feature in the same manner as Harris, which relies on second-derivatives of image intensities. Calculating the autocorrelation of the second derivative over small windows around each point gives description of the window. According to Shi and Tomasi a good corner could be described by their eigenvalues. A good corner has eigenvalues that are greater than a minimum threshold.

As the images have a low resolution, the sub-pixel corner locations are calculated as the intensity peak is almost never centred on a pixel. By fitting a parabola on the intensity of the window and find the maximum of this parabola to determining the sub-pixel location.
